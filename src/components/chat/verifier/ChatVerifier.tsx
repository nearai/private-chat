...
const ChatVerifier: React.FC = () => {
  ...
  const renderHeader = () => {
      ...
      return (
        <div className="flex items-center justify-center rounded-lg bg-muted/30 px-3 py-4 text-center text-muted-foreground text-sm">
          {t("Offline. Using cached verification results.", {
            defaultValue: "Offline. Using cached verification results.",
          })}
        </div>
      );
    }
    ...
    if (modelVerificationStatus?.error) {
    ...
    if (modelVerificationStatus?.isVerified) {
      return (
        <>
          ...
          <p className="font-normal text-sm leading-[140%] opacity-80">
            All models run in TEE (Trusted Execution Environment) — isolated
            hardware where no one can access your messages.
          </p>

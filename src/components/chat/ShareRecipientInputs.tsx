...
export const ShareRecipientInputs = ({
  ...
  const updateRecipient = (id: string, updates: Partial<RecipientInputValue>) => {
    onChange(
      recipients.map((recipient) => (recipient.id === id ? { ...recipient, ...updates } : recipient))
    );
  };
  ...
  return (
    <div className={cn("space-y-2", className)}>
      {recipients.map((recipient, index) => (
        <div key={recipient.id} className="flex flex-col gap-2 rounded-2xl border border-border/80 p-3 md:flex-row md:items-center">
          ...
          {allowMultiple && (
            <Button variant="ghost" size="icon" type="button" onClick={() => removeRecipient(recipient.id)} title="Remove recipient" className="self-start text-muted-foreground hover:text-foreground">
              <TrashIcon className="size-4" />
            </Button>
          )}
          {!allowMultiple && index === recipients.length - 1 && (
            <div className="hidden" />
          )}
        </div>
      ))}
      {allowMultiple && (
        <Button variant="ghost" type="button" className="w-full justify-start gap-2 text-muted-foreground text-sm hover:text-foreground" onClick={addRecipient}>

    ...
    <TooltipPrimitive.Portal>
      <TooltipPrimitive.Content
        ...
        className={cn(
          ...
          "z-50 w-fit max-w-96 animate-in text-balance rounded-md bg-transparent px-3 py-1.5 font-bold text-foreground text-xs",
          className
        )}
        {...props}
      >
        {children}
        <TooltipPrimitive.Arrow className="z-50 size-2.5 translate-y-[calc(-50%-2px)] rotate-45 rounded-[2px] bg-transparent fill-transparent" />
      </TooltipPrimitive.Content>
    </TooltipPrimitive.Portal>
  );
}
